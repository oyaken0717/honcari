...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	.antMatchers("/user/to_login","/user/to_register","/user/register","/about","/user/to_thank_register","/user/thank_register").permitAll() //「/」などのパスは全てのユーザに許可
...
//	.antMatchers().hasRole("USER") // /user/から始まるパスはUSER権限でログインしている場合のみアクセス可(権限設定時の「ROLE_」を除いた文字列を指定)
	...
	.loginPage("/user/to_login") // ログイン画面に遷移させるパス(ログイン認証が必要なパスを指定してかつログインされていないとこのパスに遷移される)
	.loginProcessingUrl("/login") // ログインボタンを押した際に遷移させるパス(ここに遷移させれば自動的にログインが行われる)
	.failureUrl("/user/to_login?error=true") //ログイン失敗に遷移させるパス
	...
	.logoutSuccessUrl("/user/to_login") // ログアウト後に遷移させるパス(ここではログイン画面を設定)
