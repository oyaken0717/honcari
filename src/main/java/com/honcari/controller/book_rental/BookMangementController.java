...
//	private HttpSession session;
	...
	public String toLendManagement(@AuthenticationPrincipal LoginUser loginUser, Model model) {
		...
		List<BookRental> lendBookLendingList = bookService.searchBookLendingListByLendUserId(userId);
		List<BookRental> borrowBookLendingList = bookService.searchBookLendingListByBorrowUserId(userId);

		// 貸出承認待ち本リスト
		List<BookRental> lendingBookListPendingApproval = lendBookLendingList.stream()
				.filter(bookLending -> bookLending.getLendingStatus() == 0).collect(Collectors.toList());
		// 貸し出し中本リスト
		List<BookRental> lendingBookList = lendBookLendingList.stream()
				.filter(bookLending -> bookLending.getLendingStatus() != 0).collect(Collectors.toList());
		// レンタル承認待ち本リスト
		List<BookRental> borrowingBookListPendingApproval = borrowBookLendingList.stream()
				.filter(bookLending -> bookLending.getLendingStatus() == 0).collect(Collectors.toList());
		// レンタル中本リスト
		List<BookRental> borrowingBookList = borrowBookLendingList.stream()
				.filter(bookLending -> bookLending.getLendingStatus() != 0).collect(Collectors.toList());
